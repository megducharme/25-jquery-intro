...
$('.bacon').children(1).css('color', 'red')
...
//the second parameter in the .on() methond (below) can be any selector (similar to css) to only fire the event when that thing is clicked. Here we have 'button' but it can be '.submit-btn' if you only want the function to fire when an element with the class submit-btn is clicked (otherwise it will fire when anything in #para_2 is clicked)
$('#para_2').on('click', 'button', function(event){
  console.log(event);
  // alert('I just clicked paragraph 2!');
});
